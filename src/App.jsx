...
function SchoolSettingsPage({goHome}) {
 ...
 return <><TopActions goHome={goHome}/><SectionTitle title="Dersler ve Ders Saatleri"/><div className="shared-sync-card"><strong>☁️ Cihazlar arası senkronizasyon</strong><small>Yeni değişiklikler otomatik ortak veriye kaydolur. Bu düğme yalnızca daha önce bu cihazda kalmış eski yerel ayarları ilk kez ortaklaştırmak içindir.</small><button onClick={uploadThisDevice}>Bu cihazdaki eski ayarları ortak yap</button></div><div className="school-settings-wrap">
  ...
  <section className="subjects-card"><div className="subjects-head"><div><h3>📚 Ders Tanımları</h3><small>Her ders için zemin ve metin rengini ayrı seçebilirsin.</small></div><button onClick={addSubject}>＋ Ders</button></div><div className="subject-definition-list">{subjects.map(s=><div className="subject-definition-row" key={s.id}><strong className="subject-name-preview" style={{background:s.color,color:s.textColor||'#000000'}}>{s.name}</strong><div className="subject-color-controls"><label className="subject-color-field"><span>Zemin</span><input type="color" value={s.color} onChange={e=>setSubjectColor(s.id,e.target.value)}/></label><label className="subject-color-field"><span>Metin</span><input type="color" value={s.textColor||'#000000'} onChange={e=>setSubjectTextColor(s.id,e.target.value)}/></label></div><button className="settings-edit-button" onClick={()=>renameSubject(s)}>Düzenle</button><button className="danger" onClick={()=>delSubject(s)}>Sil</button></div>)}</div></section>
 </div></>;
}
