...
function HomeworkCalendar({ tasks, onOpen }) {
  ...
  return <section className="android-home-screen calendar-screen">
    ...
    {eventDetail&&<div className="modal-backdrop" onClick={()=>setEventDetail(null)}><div className="calendar-detail-modal" onClick={e=>e.stopPropagation()}><div className="modal-head"><strong>{eventDetail.type==='exam'?'Sınav':'Takvim Notu'}</strong><button onClick={()=>setEventDetail(null)}>×</button></div><div className="calendar-detail-body"><span className="calendar-detail-date">{formatShortDate(eventDetail.date)}</span><h2>{eventDetail.subject||'Not'}</h2><p>{eventDetail.note||'Açıklama girilmemiş.'}</p></div></div></div>}
